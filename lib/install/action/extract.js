...
module.exports = function (staging, pkg, log, next) {
  ...
  next = inflight('extracting to ' + extractTo, next)
  if (!next) {
    log.silly('extract', extractTo, 'already inflight.')
    return
  }
  pacote.extract(pkg.package.name + '@' + pkg.package.version, extractTo, {
    uid: user,
    gid: group,
    ...
    pkg, staging, extractTo, andStageBundledChildren(pkg, staging, extractTo, log, next)))
}
